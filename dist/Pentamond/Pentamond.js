...
class Pentamond {
    ...
    #createGhostImageData() {
        ...
        let h = new Hand_1.Hand(this.#hand.blocks);
    ...
    #createSubImageData() {
        ...
        if (this.#hold) {
            Hand_1.Hand.createFromColor(this.#hold).blocks.forEach((b) => {
                const i = b.row * 5 + b.col - 6 - 5;
                d[i] = b.serialize();
            });
        }
        this.#next.seek().forEach((c, i) => {
            Hand_1.Hand.createFromColor(c)
                .moveBy(-(i + 1) * 3, 0)
                .blocks.forEach((b) => {
    ...
    put() {
        ...
        this.#hand = Hand_1.Hand.createFromColor(this.#next.getNext());
        this.#checkPlayable();
    }
    down() {
        if (!this.#isPlayable || this.#isEnd)
    ...
    rotate(direction) {
        ...
        // 最も早く置ける平行移動
        const validDisplacement = displacements.find(([dx, dy]) => rotated.every((block) => this.#board.canPut(block.moveBy(dx, dy))));
        if (validDisplacement) {
            const [dx, dy] = validDisplacement;
            this.#hand = new Hand_1.Hand(rotated.map((block) => block.moveBy(dx, dy)));
        }
    }
    hold() {
        if (this.#isEnd)
        ...
        if (this.#hold) {
            ;
            [this.#hold, this.#hand] = [this.#hand.getColor(), Hand_1.Hand.createFromColor(this.#hold)];
        }
        else {
    ...
    back() {
        if (this.#isEnd)
            return;
        // 戻せる手がない場合
        if (!this.#back.isAvailable())
            return;
        // 次に来る手を戻す
        this.#next.unshift(this.#hand.getColor());
        // 手を戻す
        this.#hand = Hand_1.Hand.createFromColor(this.#back.previous);
        // バックアップから復元
    ...
    #moveBy(dr, dc) {
        if (this.#hand.blocks.length === 0)
            throw new Error("handが0");
