export class Input {
    ...
    static keyboard = {
        ...
        longPressed: new Set<string>(),
        pushed: new Set<string>(),
        upped: new Set<string>(),
        get(key: string) {
    ...
    static #pressTiming = new Map<string, number>()
    static #lastPublishTiming = performance.now()
    static #longPressInterval = new Map<string, number>([
        ["KeyC", Infinity],
        ["KeyV", Infinity],
    ])
    ...
    static #handleKeyDown(e: { code: string }) {
    ...
    static #handleKeyUp(e: { code: string }) {
        if (!this.available) return
    ...
    static update() {
        ...
        if (isLongPressDetected) this.#lastPublishTiming = now
